...
class FormulaUpdate(BaseModel):
    ...
    # Item 8 - Modo de Preparo estruturado (lista de passos)
    # Aceita List[str] (legado) ou List[Dict] com {ordem, fase, descricao, temperatura_c, tempo_minutos, equipamento, rpm, alerta}
    modo_preparo: Optional[List[Any]] = None
    # Item 4 - Parametros de controle da Ficha Tecnica
    ph_min: Optional[float] = None
    ph_max: Optional[float] = None
    viscosidade_min: Optional[float] = None
    viscosidade_max: Optional[float] = None
    densidade_min: Optional[float] = None
    densidade_max: Optional[float] = None
    aspecto: Optional[str] = None
    cor: Optional[str] = None
    odor: Optional[str] = None
    rendimento_kg_por_lote: Optional[float] = None
...
async def formula_bank(
    ...
    for formula in formulas:
        ...
        _frag_phases = {"fragrancia", "fragrância", "fragrance", "frag"}
        fragrance_pct = round(sum(
            (it.get("percentage") or 0) for it in formula_items
            if (str(it.get("phase", "")).strip().lower() in _frag_phases)
            or any(kw in str(it.get("ingredient_name", "")).lower() for kw in _frag_kws)
        ), 4)
...
def _build_cost_versions_response(doc: dict, user: dict, formula_cost_auto: float = 0.0) -> dict:
    ...
    Comercial roles  → ONLY consolidated total_final from v2 (no breakdown, no v1).
    """
    if not doc:
        doc = {}
    ...
    # Comercial-only view (vendedor, sales_ops, sucesso_cliente): only final total of v2
    if has_role(user, {"vendedor", "sales_ops", "sucesso_cliente"}) and not has_role(user, PD_FULL):
        v2_is_final = v2_raw and v2_raw.get("status") == "finalizado"
        return {
            "v1": None,
            "v2": None,
            "total_final": total_final if v2_is_final else None,
            "v2_status": (v2_raw or {}).get("status"),
            "updated_at": doc.get("updated_at"),
...
async def get_cost_versions(dev_id: str, request: Request):
    ...
    if not dev:
        raise HTTPException(status_code=404, detail="Desenvolvimento não encontrado")
    # Allow comercial roles to view the consolidated final cost (Item 5)
    require_roles(user, PD_READ | {"vendedor", "sucesso_cliente"})
...
async def get_ficha_tecnica_ui(req_id: str, request: Request):
    ...
    if dev:
        ...
        if formula:
            items = await db.pd_formula_items.find({"formula_id": formula["id"]}, {"_id": 0}).to_list(200)
    # Item 4 — Ficha Técnica é documento operacional. NUNCA retornar custos.
    # Remove campos financeiros independente do papel para garantir higiene da FT.
    _COST_FIELDS = ("price_per_kg", "cost_brl", "cost_kg_usd", "cost_percentage")
    items_no_cost = [
        {k: v for k, v in it.items() if k not in _COST_FIELDS}
        for it in items
    ]
...
@pd_router.get("/catalog/search")
async def search_catalog_for_formula(request: Request, q: str = ""):
    """
    Item 2 — Busca de MP no banco de custos para autocomplete na formulação.
    Retorna nome, INCI, fornecedores com preços ordenados (mais barato primeiro)
    e semáforo de cor por preço (verde → vermelho).
    Combina dados do catalog (fornecedores embarcados) com homologacao_mps quando disponível.
    """
    user = await get_current_user(request)
    tenant_id = user["tenant_id"]
    ...
    catalog_items = await db.pd_catalog.find(query, {"_id": 0}).limit(20).to_list(20)
    if not catalog_items:
        return []

    catalog_ids = [c.get("id") for c in catalog_items]
    homologacoes = await db.homologacao_mps.find(
        {"tenant_id": tenant_id, "catalog_id": {"$in": catalog_ids}},
        {"_id": 0},
    ).to_list(500)
    homol_by_catalog: Dict[str, List[Dict[str, Any]]] = {}
    for h in homologacoes:
        homol_by_catalog.setdefault(h.get("catalog_id"), []).append(h)

    enriched = []
    for item in catalog_items:
        fornecedores_raw = list(item.get("fornecedores") or [])
        default_fornecedor = (item.get("fornecedor") or "").strip()
        if default_fornecedor and not any(
            (f.get("nome") or "").strip().lower() == default_fornecedor.lower() for f in fornecedores_raw
        ):
            fornecedores_raw.insert(0, {
                "nome": default_fornecedor,
                "preco_rs_kg": item.get("preco_rs_kg", 0),
                "moeda": item.get("moeda", "BRL"),
            })

        homol_list = homol_by_catalog.get(item.get("id"), [])
        homol_map = {(h.get("fornecedor_nome") or "").strip().lower(): h for h in homol_list}

        fornecedores_validos = [f for f in fornecedores_raw if (f.get("preco_rs_kg") or 0) > 0]
        fornecedores_validos.sort(key=lambda f: f.get("preco_rs_kg") or 0)

        total = len(fornecedores_validos)
        fornecedores_enriched = []
        for idx, f in enumerate(fornecedores_validos):
            f_nome = (f.get("nome") or "").strip()
            homol = homol_map.get(f_nome.lower())
            fornecedores_enriched.append({
                "id": f.get("id") or f"{item.get('id')}-{idx}",
                "nome": f_nome,
                "preco_rs_kg": f.get("preco_rs_kg"),
                "moeda": f.get("moeda", "BRL"),
                "codigo": f.get("codigo"),
                "semaforo": _supplier_semaforo_color_inline(idx, total),
                "homologado": bool(homol and homol.get("status") == "homologada"),
                "status_homologacao": (homol or {}).get("status"),
            })
        ...
        enriched.append({
            ...
            "fornecedores": fornecedores_enriched,
            "fornecedores_count": len(fornecedores_enriched),
        })

    q_lower = q.lower()
    enriched.sort(key=lambda x: (
        0 if x["nome"].lower().startswith(q_lower) else (1 if q_lower in x["nome"].lower() else 2),
        x["nome"].lower(),
    ))
    return enriched


def _supplier_semaforo_color_inline(idx: int, total: int) -> str:
    """Semáforo de cor baseado na posição do fornecedor por preço."""
    if total <= 1:
        return "verde"
...
# ============================================================
# ITEM 2 — (impl. movida para antes de /catalog/{item_id} para evitar conflito de rota)
# ============================================================


# ============================================================
# ITEM 3 — ELABORAÇÃO EM LOTE + ORDEM DE MANIPULAÇÃO
# ============================================================

class CriarOrdemManipulacaoRequest(BaseModel):
    sample_id: str
    variacao_ids: List[str]
    formula_base_id: str
    volume_amostra_ml: float = 15.0
    fator_perda_pct: float = 10.0
    observacoes: Optional[str] = None


class OrdemManipulacaoStatusUpdate(BaseModel):
    status: str  # rascunho | emitida | executada


async def _gerar_numero_om(tenant_id: str) -> str:
    ano = datetime.now(timezone.utc).year
    count = await db.ordens_manipulacao.count_documents({
        "tenant_id": tenant_id,
        "numero_om": {"$regex": f"^OM-{ano}-"}
    })
    return f"OM-{ano}-{str(count + 1).zfill(4)}"


@pd_router.post("/ordens-manipulacao")
async def criar_ordem_manipulacao(body: CriarOrdemManipulacaoRequest, request: Request):
    """
    Item 3 — Cria Ordem de Manipulação para um lote de variações que compartilham
    a mesma base de fórmula. Calcula quantidade necessária de cada MP da base
    + fragrâncias específicas por variação.
    """
    user = await get_current_user(request)
    require_roles(user, PD_WRITE)
    tenant_id = user["tenant_id"]
    ...
    # Buscar fórmula base
    formula_base = await db.pd_formulas.find_one(
        {"id": body.formula_base_id}, {"_id": 0}
    )
    if not formula_base:
        raise HTTPException(status_code=404, detail="Fórmula base não encontrada")

    # Confirmar tenant via development
    dev = await db.pd_developments.find_one(
        {"id": formula_base.get("development_id"), "tenant_id": tenant_id}, {"_id": 0}
    )
    if not dev:
        raise HTTPException(status_code=403, detail="Fórmula não pertence ao seu tenant")

    # Itens da fórmula
    formula_items = await db.pd_formula_items.find(
        {"formula_id": body.formula_base_id}, {"_id": 0}
    ).to_list(500)

    # Buscar amostra (CRM)
    sample = await db.crm_samples.find_one(
        {"id": body.sample_id, "tenant_id": tenant_id}, {"_id": 0}
    )
    if not sample:
        raise HTTPException(status_code=404, detail="Amostra não encontrada")

    variacoes_selecionadas = [
        v for v in sample.get("variacoes", []) if v.get("id") in body.variacao_ids
    ]
    if not variacoes_selecionadas:
        raise HTTPException(status_code=404, detail="Nenhuma variação válida encontrada")

    n_variacoes = len(variacoes_selecionadas)

    # Volume total da base = N_variacoes × volume_amostra × (1 + fator_perda)
    volume_por_variacao_com_perda = body.volume_amostra_ml * (1 + body.fator_perda_pct / 100.0)
    volume_total_base_ml = n_variacoes * volume_por_variacao_com_perda

    # Percentual de fragrância máximo entre as variações (a "base" representa o resto)
    pct_fragrancia_max = max(
        (float(v.get("percentual_fragrancia") or 0) for v in variacoes_selecionadas),
        default=0.0,
    )

    # Filtrar itens da BASE (excluir fragrâncias - elas são por variação)
    _frag_phases = {"fragrancia", "fragrância", "fragrance", "frag"}
    _frag_kws = ["fragr", "essência", "essencia", "perfum", "aroma"]
    itens_base = [
        item for item in formula_items
        if (str(item.get("phase", "")).strip().lower() not in _frag_phases)
        and not any(kw in str(item.get("ingredient_name", "")).lower() for kw in _frag_kws)
    ]

    linhas_base = []
    for item in itens_base:
        pct_item = float(item.get("percentage", 0) or 0) / 100.0
        # Quantidade aproximada em gramas (densidade=1 para cosméticos como base)
        quantidade_g = volume_total_base_ml * pct_item
        linhas_base.append({
            "tipo": "base",
            "ingredient_name": item.get("ingredient_name", ""),
            "inci": item.get("inci", ""),
            "fornecedor": item.get("fornecedor", ""),
            "percentage_formula": item.get("percentage", 0),
            "quantidade_g": round(quantidade_g, 3),
            "fase": item.get("phase", ""),
            "observacao": None,
        })

    # Linhas de fragrância - uma por variação
    linhas_fragrancias = []
    for variacao in variacoes_selecionadas:
        pct_frag = float(variacao.get("percentual_fragrancia") or 0) / 100.0
        quantidade_frag_g = volume_por_variacao_com_perda * pct_frag
        linhas_fragrancias.append({
            "tipo": "fragrancia",
            "variacao_codigo": variacao.get("numero_completo", variacao.get("codigo", "")),
            "variacao_id": variacao.get("id"),
            "ingredient_name": f"Fragrância — {variacao.get('referencia_fragrancia', variacao.get('numero_completo', ''))}",
            "referencia": variacao.get("referencia_fragrancia", ""),
            "percentage_formula": variacao.get("percentual_fragrancia", 0),
            "quantidade_g": round(quantidade_frag_g, 3),
            "fase": "Fragrância",
            "observacao": variacao.get("observacoes_especificas"),
        })

    numero_om = await _gerar_numero_om(tenant_id)
    ordem = {
        "id": new_id(),
        "numero_om": numero_om,
        "tenant_id": tenant_id,
        "sample_id": body.sample_id,
        "variacao_ids": body.variacao_ids,
        "formula_base_id": body.formula_base_id,
        "n_variacoes": n_variacoes,
        "volume_amostra_ml": body.volume_amostra_ml,
        "fator_perda_pct": body.fator_perda_pct,
        "volume_total_base_ml": round(volume_total_base_ml, 2),
        "volume_por_variacao_com_perda_ml": round(volume_por_variacao_com_perda, 2),
        "pct_fragrancia_max": pct_fragrancia_max,
        "linhas_base": linhas_base,
        "linhas_fragrancias": linhas_fragrancias,
        "status": "rascunho",  # rascunho | emitida | executada
        "criado_por": user["id"],
        "criado_por_nome": user.get("name", ""),
        "created_at": now_iso(),
        "updated_at": now_iso(),
        "observacoes": body.observacoes,
        "nome_produto": sample.get("nome_amostra") or sample.get("produto") or "",
        "cliente": sample.get("cliente") or sample.get("client_name") or "",
    }
    await db.ordens_manipulacao.insert_one(ordem)
    ordem.pop("_id", None)
    return ordem


@pd_router.get("/ordens-manipulacao")
async def listar_ordens_manipulacao(
    request: Request,
    sample_id: Optional[str] = None,
    status: Optional[str] = None,
    formula_base_id: Optional[str] = None,
):
    user = await get_current_user(request)
    require_roles(user, PD_READ | COMERCIAL_FULL_FALLBACK)
    query = {"tenant_id": user["tenant_id"]}
    if sample_id:
    ...
    if status:
        query["status"] = status
    if formula_base_id:
        query["formula_base_id"] = formula_base_id
    ordens = await db.ordens_manipulacao.find(query, {"_id": 0}).sort("created_at", -1).to_list(500)
    return ordens


@pd_router.get("/ordens-manipulacao/{om_id}")
async def get_ordem_manipulacao(om_id: str, request: Request):
    user = await get_current_user(request)
    ordem = await db.ordens_manipulacao.find_one(
        {"id": om_id, "tenant_id": user["tenant_id"]}, {"_id": 0}
    )
    if not ordem:
        raise HTTPException(status_code=404, detail="Ordem de Manipulação não encontrada")
    return ordem


@pd_router.put("/ordens-manipulacao/{om_id}/status")
async def atualizar_status_om(om_id: str, body: OrdemManipulacaoStatusUpdate, request: Request):
    user = await get_current_user(request)
    require_roles(user, PD_WRITE)
    if body.status not in ("rascunho", "emitida", "executada"):
        raise HTTPException(status_code=422, detail="status inválido")
    result = await db.ordens_manipulacao.update_one(
        {"id": om_id, "tenant_id": user["tenant_id"]},
        {"$set": {
            "status": body.status,
            "updated_at": now_iso(),
            "status_atualizado_por": user.get("name", ""),
        }},
    )
    if result.matched_count == 0:
        raise HTTPException(status_code=404, detail="OM não encontrada")
    return await get_ordem_manipulacao(om_id, request)


@pd_router.delete("/ordens-manipulacao/{om_id}")
async def deletar_ordem_manipulacao(om_id: str, request: Request):
    user = await get_current_user(request)
    require_roles(user, PD_WRITE)
    ordem = await db.ordens_manipulacao.find_one(
        {"id": om_id, "tenant_id": user["tenant_id"]}, {"_id": 0}
    ...
    if ordem.get("status") == "executada":
        raise HTTPException(status_code=409, detail="Não é possível excluir uma OM já executada")
    await db.ordens_manipulacao.delete_one({"id": om_id, "tenant_id": user["tenant_id"]})
    return {"deleted": True}


@pd_router.get("/ordens-manipulacao/{om_id}/pdf")
async def exportar_om_pdf(om_id: str, request: Request):
    """Item 3 — PDF da Ordem de Manipulação."""
    from reportlab.lib.pagesizes import A4
    from reportlab.lib import colors as rl_colors
    from reportlab.lib.units import mm
    from reportlab.platypus import SimpleDocTemplate, Table, TableStyle, Paragraph, Spacer, HRFlowable
    ...
    styles = getSampleStyleSheet()
    title_style = ParagraphStyle('OMTitle', parent=styles['Title'], fontSize=18, spaceAfter=4, textColor=rl_colors.HexColor('#0A0A0B'))
    subtitle_style = ParagraphStyle('OMSubtitle', parent=styles['Normal'], fontSize=10, textColor=rl_colors.HexColor('#737373'), spaceAfter=6)
    heading_style = ParagraphStyle('OMHeading', parent=styles['Heading2'], fontSize=12, spaceAfter=5, spaceBefore=10, textColor=rl_colors.HexColor('#0A0A0B'))
    normal_style = ParagraphStyle('OMNormal', parent=styles['Normal'], fontSize=9, leading=12)
    small_style = ParagraphStyle('OMSmall', parent=styles['Normal'], fontSize=8, textColor=rl_colors.HexColor('#737373'))

    elements = []
    elements.append(Paragraph("ORDEM DE MANIPULAÇÃO", title_style))
    elements.append(Paragraph(f"KURYOS — Pesquisa & Desenvolvimento", subtitle_style))
    elements.append(HRFlowable(width="100%", thickness=1, color=rl_colors.HexColor('#E5E5E5'), spaceAfter=6))

    # Cabeçalho
    head_data = [
        ["OM:", ordem.get("numero_om", "—"), "Status:", (ordem.get("status") or "rascunho").upper()],
        ["Produto:", ordem.get("nome_produto", "—"), "Cliente:", ordem.get("cliente", "—")],
        ["Data:", (ordem.get("created_at") or "")[:10], "Criado por:", ordem.get("criado_por_nome", "—")],
    ]
    head_table = Table(head_data, colWidths=[22 * mm, 70 * mm, 22 * mm, 60 * mm])
    head_table.setStyle(TableStyle([
        ('FONTNAME', (0, 0), (0, -1), 'Helvetica-Bold'),
        ('FONTNAME', (2, 0), (2, -1), 'Helvetica-Bold'),
        ('FONTSIZE', (0, 0), (-1, -1), 9),
        ('TEXTCOLOR', (0, 0), (0, -1), rl_colors.HexColor('#737373')),
        ('TEXTCOLOR', (2, 0), (2, -1), rl_colors.HexColor('#737373')),
        ('BOTTOMPADDING', (0, 0), (-1, -1), 3),
    ...
    # Resumo
    resumo = (
        f"Variações: <b>{ordem.get('n_variacoes', 0)}</b> &nbsp;|&nbsp; "
        f"Volume/amostra: <b>{ordem.get('volume_amostra_ml', 0)} mL</b> &nbsp;|&nbsp; "
        f"Fator perda: <b>{ordem.get('fator_perda_pct', 0)}%</b> &nbsp;|&nbsp; "
        f"Vol. base total: <b>{ordem.get('volume_total_base_ml', 0)} mL</b>"
    )
    elements.append(Paragraph(resumo, normal_style))

    # Tabela BASE
    elements.append(Paragraph("BASE COMUM (sem fragrância)", heading_style))
    base_rows = [["Ingrediente", "INCI", "Fornecedor", "% Fórmula", "Quantidade (g)", "Fase"]]
    for l in ordem.get("linhas_base", []):
        base_rows.append([
            l.get("ingredient_name", ""),
            l.get("inci", "") or "—",
            l.get("fornecedor", "") or "—",
            f"{l.get('percentage_formula', 0)}%",
            f"{l.get('quantidade_g', 0)} g",
            l.get("fase", "") or "—",
        ])
    base_table = Table(base_rows, colWidths=[48 * mm, 32 * mm, 28 * mm, 20 * mm, 28 * mm, 18 * mm])
    base_table.setStyle(TableStyle([
        ...
        ('FONTSIZE', (0, 0), (-1, -1), 8),
        ('ALIGN', (3, 0), (4, -1), 'RIGHT'),
        ('GRID', (0, 0), (-1, -1), 0.4, rl_colors.HexColor('#E5E5E5')),
    ...
    # Tabela FRAGRÂNCIAS
    elements.append(Paragraph("FRAGRÂNCIAS POR VARIAÇÃO", heading_style))
    frag_rows = [["Variação", "Referência", "%", "Quantidade (g)", "Obs."]]
    for l in ordem.get("linhas_fragrancias", []):
        frag_rows.append([
            l.get("variacao_codigo", ""),
            l.get("referencia", "") or "—",
            f"{l.get('percentage_formula', 0)}%",
            f"{l.get('quantidade_g', 0)} g",
            l.get("observacao", "") or "—",
        ])
    frag_table = Table(frag_rows, colWidths=[30 * mm, 48 * mm, 16 * mm, 28 * mm, 52 * mm])
    ...
    if ordem.get("observacoes"):
        ...
        elements.append(Paragraph(f"<b>Observações:</b> {ordem['observacoes']}", normal_style))

    elements.append(Spacer(1, 25))
    elements.append(HRFlowable(width=60 * mm, thickness=0.5, color=rl_colors.HexColor('#737373'), spaceAfter=2))
    elements.append(Paragraph("Formulador Responsável", small_style))

    doc.build(elements)
    buffer.seek(0)
    filename = f"{ordem.get('numero_om', om_id)}.pdf"
...
# Helper para roles de comercial (importado dinamicamente para não quebrar rbac.py existente)
COMERCIAL_FULL_FALLBACK = {"admin", "vendedor", "sales_ops", "sucesso_cliente"}


# ============================================================
# ITEM 4 — FICHA TÉCNICA SEM VALORES FINANCEIROS (JSON endpoint)
# ============================================================

@pd_router.get("/requests/{req_id}/ficha-tecnica-data")
async def get_ficha_tecnica_data(req_id: str, request: Request):
    """
    Item 4 — Retorna estrutura da Ficha Técnica (JSON) SEM dados financeiros.
    Para uso de Manipulação, Produção e CQ. Não contém price_per_kg, cost_brl,
    cost_kg_usd nem qualquer campo de custo.
    """
    user = await get_current_user(request)
    tenant_id = user["tenant_id"]
    ...
    if not pd_req:
        ...
        if pd_card and pd_card.get("pd_request_id"):
            pd_req = await db.pd_requests.find_one(
                {"id": pd_card["pd_request_id"], "tenant_id": tenant_id}, {"_id": 0}
            )
    if not pd_req:
        raise HTTPException(status_code=404, detail="Solicitação não encontrada")

    dev = await db.pd_developments.find_one(
        {"pd_request_id": pd_req["id"], "tenant_id": tenant_id}, {"_id": 0}
    )
    if not dev:
        raise HTTPException(status_code=404, detail="Development não encontrado")

    formulas = await db.pd_formulas.find(
        {"development_id": dev["id"]}, {"_id": 0}
    ).sort("version", -1).to_list(5)
    formula = formulas[0] if formulas else None

    composicao_sem_custo: List[Dict[str, Any]] = []
    if formula:
        items = await db.pd_formula_items.find(
            {"formula_id": formula["id"]},
            {"_id": 0, "price_per_kg": 0, "cost_brl": 0, "cost_kg_usd": 0, "cost_percentage": 0},
        ).to_list(500)
        for item in items:
            composicao_sem_custo.append({
                "ingredient_name": item.get("ingredient_name", ""),
                "inci": item.get("inci", ""),
                "fornecedor": item.get("fornecedor", ""),
    ...
    return {
        "numero_ft": f"FT-{pd_req['id'][:8].upper()}",
        "versao": f"v{(formula or {}).get('version', 1)}",
        "produto": {
            "nome_tecnico": pd_req.get("project_name", ""),
            "nome_comercial": pd_req.get("commercial_name") or pd_req.get("client_name", ""),
            "categoria": pd_req.get("category", ""),
            "formulador": dev.get("assigned_to_name", "") or (formula or {}).get("created_by_name", ""),
            "data": ((formula or {}).get("created_at") or "")[:10],
        },
        "composicao": composicao_sem_custo,
        "modo_preparo": (formula or {}).get("modo_preparo") or [],
        "parametros_controle": {
            "ph_min": (formula or {}).get("ph_min"),
            "ph_max": (formula or {}).get("ph_max"),
            "viscosidade_min": (formula or {}).get("viscosidade_min"),
            "viscosidade_max": (formula or {}).get("viscosidade_max"),
            "densidade_min": (formula or {}).get("densidade_min"),
            "densidade_max": (formula or {}).get("densidade_max"),
            "aspecto": (formula or {}).get("aspecto", ""),
            "cor": (formula or {}).get("cor", ""),
            "odor": (formula or {}).get("odor", ""),
        },
        "rendimento": {
            "por_lote_kg": (formula or {}).get("rendimento_kg_por_lote"),
            "fator_perda_pct": (formula or {}).get("indice_perdas", 0),
        },
        "observacoes_tecnicas": (formula or {}).get("observacoes_tecnicas", ""),
    }
...
class RetrocederPDCardRequest(BaseModel):
    status_destino: str
    justificativa: str


class DecisaoRetrocessoRequest(BaseModel):
    decisao: str  # "aprovado" | "negado"
    observacao: Optional[str] = None


@pd_router.post("/cards/{card_id}/retroceder")
async def retroceder_pd_card(card_id: str, body: RetrocederPDCardRequest, request: Request):
    """
    Item 6 — Solicitar retrocesso de card P&D com justificativa.
    - admin/lider_pd: executa imediatamente
    - formulador/outros: cria solicitação aguardando aprovação do líder
    """
    user = await get_current_user(request)
    require_roles(user, PD_FULL)
    tenant_id = user["tenant_id"]
    ...
    if not body.justificativa or len(body.justificativa.strip()) < 10:
        raise HTTPException(status_code=422, detail="Justificativa obrigatória (mínimo 10 caracteres)")

    pd_card = await db.pd_cards.find_one({"id": card_id, "tenant_id": tenant_id}, {"_id": 0})
    if not pd_card:
        raise HTTPException(status_code=404, detail="Card não encontrado")

    # Importar status válidos do crm_routes (referência)
    valid_destinos = {
        "solicitado", "em_desenvolvimento", "em_testes",
        "aguardando_aprovacao", "aprovado_internamente",
        "entregue_ao_comercial", "retrabalho_interno",
    }
    if body.status_destino not in valid_destinos:
        raise HTTPException(status_code=422, detail=f"status_destino inválido: {body.status_destino}")

    # Admin / Líder P&D aplicam direto
    if has_role(user, {"admin", "lider_pd"}):
        await db.pd_cards.update_one(
            {"id": card_id, "tenant_id": tenant_id},
            {
                "$set": {
                    "status_pd": body.status_destino,
                    "updated_at": now_iso(),
                },
                "$push": {
                    "historico_retrocessos": {
                        "de": pd_card.get("status_pd"),
                        "para": body.status_destino,
        ...
        return {
            "status": "retrocedido",
            "novo_status": body.status_destino,
            "card_id": card_id,
        }

    # Outros perfis: criar solicitação pendente
    solicitacao = {
        "id": new_id(),
        "tenant_id": tenant_id,
        "pd_card_id": card_id,
        "status_atual": pd_card.get("status_pd"),
        "status_destino": body.status_destino,
        "justificativa": body.justificativa,
        "solicitado_por": user["id"],
        "solicitado_por_nome": user.get("name", ""),
        "status_solicitacao": "pendente",
        "created_at": now_iso(),
    }
    await db.pd_retrocessos_pendentes.insert_one(solicitacao)
    solicitacao.pop("_id", None)
    return {
        "status": "aguardando_aprovacao",
        "message": "Solicitação de retrocesso criada. Aguardando aprovação do Líder P&D.",
        "solicitacao": solicitacao,
    }


@pd_router.get("/retrocessos/pendentes")
async def listar_retrocessos_pendentes(request: Request):
    user = await get_current_user(request)
    require_roles(user, {"admin", "lider_pd"})
    pendentes = await db.pd_retrocessos_pendentes.find(
        {"tenant_id": user["tenant_id"], "status_solicitacao": "pendente"},
        {"_id": 0},
    ).sort("created_at", -1).to_list(200)
    return pendentes


@pd_router.put("/retrocessos/{solicitacao_id}/decisao")
async def aprovar_retrocesso(solicitacao_id: str, body: DecisaoRetrocessoRequest, request: Request):
    user = await get_current_user(request)
    require_roles(user, {"admin", "lider_pd"})
    if body.decisao not in ("aprovado", "negado"):
        raise HTTPException(status_code=422, detail="decisao deve ser 'aprovado' ou 'negado'")

    solicitacao = await db.pd_retrocessos_pendentes.find_one(
        {"id": solicitacao_id, "tenant_id": user["tenant_id"]}, {"_id": 0}
    )
    if not solicitacao:
        raise HTTPException(status_code=404, detail="Solicitação não encontrada")
    if solicitacao.get("status_solicitacao") != "pendente":
        raise HTTPException(status_code=409, detail="Solicitação já foi decidida")

    await db.pd_retrocessos_pendentes.update_one(
        {"id": solicitacao_id},
        {"$set": {
            "status_solicitacao": body.decisao,
            "decidido_por": user.get("name", ""),
            "decidido_em": now_iso(),
            "observacao_decisao": body.observacao or "",
        }},
    )

    if body.decisao == "aprovado":
        await db.pd_cards.update_one(
            {"id": solicitacao["pd_card_id"], "tenant_id": user["tenant_id"]},
            {
                "$set": {
                    "status_pd": solicitacao["status_destino"],
                    "updated_at": now_iso(),
                },
                "$push": {
                    "historico_retrocessos": {
                        "de": solicitacao["status_atual"],
                        "para": solicitacao["status_destino"],
                        "justificativa": solicitacao["justificativa"],
                        "aprovado_por": user.get("name", ""),
                        "solicitado_por": solicitacao.get("solicitado_por_nome"),
                        "data": now_iso(),
                    }
                },
            },
    ...
    return {"status": body.decisao, "solicitacao_id": solicitacao_id}
